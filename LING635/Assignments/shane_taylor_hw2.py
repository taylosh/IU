...
## My implementation
# Iterate through entire test dataset
for i in range(len(test_dataset)):
    ...
    # Get the prediction text (remove special tokens for scoring)
    prediction_text = pred[0][0].replace('<eng><asr><notimestamps>', '').strip()
    reference_text = sample['text_raw']

    # Store for scoring
    hyps.append(prediction_text)
...
# Compute WER and CER for test datasets
wer_score = jiwer.wer(refs, hyps)
cer_score = jiwer.cer(refs, hyps)

print(f"Pre-trained Model - Average WER: {wer_score:.2%}")
print(f"Pre-trained Model - Average CER: {cer_score:.2%}")

# Print a few examples to see what's happening
print("\nFirst 5 examples:")
for i in range(min(5, len(hyps))):
    print(f"Ref: '{refs[i]}' | Hyp: '{hyps[i]}'")
...
for i in range(len(test_dataset)):
    ...
    # Get the prediction text (remove the special tokens for scoring)
    prediction_text = pred[0][0].replace('<eng><asr><notimestamps>', '').strip()
    reference_text = sample['text_raw']
...
print(f"Fine-tuned Model - Average WER: {wer_score:.2%}")
print(f"Fine-tuned Model - Average CER: {cer_score:.2%}")

# Optional: Print a few examples to see what's happening
print("\nFirst 5 examples:")
for i in range(min(5, len(hyps))):
    print(f"Ref: '{refs[i]}' | Hyp: '{hyps[i]}'")
